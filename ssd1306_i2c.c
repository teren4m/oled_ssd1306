...
int getLine(int linePosition, unsigned char c)
{
    if (linePosition == FONT_WIDTH)
        return 0x0;
    else
        return pgm_read_byte(font + (c * FONT_WIDTH) + linePosition);
}

int isClip(int x, int y, int size){
    return ((x >= WIDTH) ||	     // Clip right
